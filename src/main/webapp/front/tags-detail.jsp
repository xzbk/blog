					...
					<div class="page">
						<c:choose>
							<c:when test="${data.currentPage>1 }">
								<a class="page-number prev" href="<%=basePath%>article/queryAllByTagId?uid=${user.id }&tagId=${tagId }&currentPage=${data.currentPage-1 }"><</a>
							</c:when>
							<c:otherwise>
						...
						<c:forEach begin="1" end="${totalPage }" var="page">
							<c:choose>
								...
								<c:otherwise>
									<a class="page-number" href="<%=basePath%>article/queryAllByTagId?uid=${user.id }&tagId=${tagId }&currentPage=${page }">${page }</a>
								</c:otherwise>
							</c:choose>                            	
						</c:forEach>
						<c:choose>
							<c:when test="${data.currentPage<totalPage }">
								<a class="page-number next" href="<%=basePath%>article/queryAllByTagId?uid=${user.id }&tagId=${tagId }&currentPage=${data.currentPage+1 }">></a>
							</c:when>
							<c:otherwise>
